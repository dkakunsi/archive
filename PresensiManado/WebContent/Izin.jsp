...
<title>Sistem Presensi - Data Izin</title>
</head>

<body>
	<%@ page import="edu.dkakunsi.ta.presensi.model.entity.Izin" %>
	    ...
	    <div id="table">
        	...
        	<table border="1">
	    		<tr>
		  			...
		  			<th width="150" scope="col">JAM KELUAR</th>
		  			<th width="150" scope="col">JAM MASUK</th>
		  			<th width="150" scope="col">KETERANGAN</th>
        		</tr>
        		<% @SuppressWarnings("unchecked") List<Izin> ls = (List<Izin>)request.getAttribute("rsp"); %>
        		<% for(Izin i : ls) { %>
	        		<tr>
    	    		    <td><%= i.getId().getNip() %></td>
        	  			<td><%= i.getPresensi().getPegawai().getNama() %></td>
    	    		    <td><%= i.getId().getTanggal() %></td>
          				<td><%= i.getKeluar() %></td>
       				  	<td><%= i.getMasuk() %></td>
       				  	<td><%= i.getKeterangan() %></td>
       			    </tr>
       		  <% } %>
   		  </table>
        </div>
